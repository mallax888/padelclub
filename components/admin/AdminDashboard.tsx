    ...
    <div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[
          ...
          { label: 'Revenue', value: formatNzd(revenue) },
        ].map(({ label, value }) => (
      ...
      <div className="flex gap-1 mb-4 border-b border-gray-200">
        {(['bookings', 'members', 'courts'] as const).map(t => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={cn('px-4 py-2 text-sm capitalize border-b-2 -mb-px transition-colors',
              tab === t ? 'border-brand-400 text-brand-600 font-medium' : 'border-transparent text-gray-500 hover:text-gray-700'
            )}
          >
            {t}
          </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {courts.map(c => (
            <div key={c.id} className="card">
              ...
              <div className="mt-3 text-sm text-gray-500">{formatNzd(c.price_per_hour)}/hr · {c.surface}</div>
