...
export default function MembershipPanel({
  ...
  const currentMem = MEMBERSHIP_CONFIG[profile.membership_tier]
  ...
  const handlePurchase = async () => {
    ...
    if (!txErr) {
      await supabase
        .from('profiles')
        .update({ credits: profile.credits + pack.sessions })
        .eq('id', profile.id)
        ...
        {Object.values(MEMBERSHIP_CONFIG).map(mem => (
          <div
            ...
            className={cn(
              ...
              profile.membership_tier === mem.id && 'bg-brand-50'
            )}
          >
            {mem.featured && (
            ...
            <button
              ...
              disabled={profile.membership_tier === mem.id || upgrading}
              onClick={() => handleUpgrade(mem.id)}
            >
              {profile.membership_tier === mem.id ? 'Current plan' : `Select ${mem.name}`}
            </button>
          </div>
        ))}
      </div>
