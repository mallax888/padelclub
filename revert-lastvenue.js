...
const target1 = `    lastVenueSlug = null,
  }: {
    courts: Court[]
    profile: Profile
    userId: string
...
const target2 = `  const initialVenue = lastVenueSlug ? VENUES.find(v => v.slug === lastVenueSlug) ?? null : null
...
const replacement2 = `  const [step, setStep] = useState<Step>('country')
...
console.log('Reverted to always country:', c.includes("useState<Step>('country')"));
console.log('lastVenueSlug prop removed:', !c.includes('lastVenueSlug'));
